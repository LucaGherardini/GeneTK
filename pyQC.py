...
def sex_discrepancy(step, rm_tmp, sex_imputation):
    ...
    if sex_imputation == 0:
        ...
        if rm_tmp == 'Y': remove(f"{f}_{step}")
        step += 1 
        command(f"mv {f}_{step}.log {f}_{step}.hh plink.sexcheck Phase_{phase}/")
...
def population_stratification(step, rm_tmp):
    ...
    if rscripts:
        ...
        command(f"mv {f}_{step}.log uncorresponding_SNPs.txt SNPs_for_exclusion.txt Phase_{phase}/")
        if rm_tmp == 'Y': remove(f"{f}_{pop_step}")
        pop_step += 1
...
def mds_covariates(step):
    ...
    command(f"mv {f}_{step}.cluster* {f}_{step}.genome {f}_{step}.log {f}_{step}.mds Phase_{phase}/")
